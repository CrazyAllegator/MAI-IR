...
void Chart::draw(HDC hdc, const RECT & clientRect)
{
	...
	const String xMaxLabel = uIntToStr(pow(10,xMax));
	const String yMaxLabel = uIntToStr(pow(10,yMax));
	...
	for (size_t value=0; value <= xMax; ++value)
	{
		...
		const auto label = uIntToStr(pow(10,value)); 
	...
	for (size_t value = 0; value <= yMax; ++value)
	{
	...
	for (size_t x = 0; x < frequencies_.size(); ++x)
	{
		...
		POINT p{ center.x + kx * log10(x+1), center.y - ky * log10(frequencies_[x]) };
		if (x == 0)
